...
import { IPIP50, IPIP100, LIKERT_LABELS, scoreBigFive } from "@/lib/assessments";
...
export default function BigFiveTest({
  ...
  searchParams: { long?: string };
}) {
  const long = searchParams.long === "1";
  const items = long ? IPIP100 : IPIP50;
  ...
  const preview = useMemo(() => (answered >= 10 ? scoreBigFive(answers, items) : null), [answers, answered, items]);

  function submit() {
    const scores = scoreBigFive(answers, items);
    start(async () => {
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="mt-1 text-slate-400">
          🟢 Evidenzbasiert · {long ? "IPIP-100 (Langversion, 20 Items/Dimension — höhere Genauigkeit)" : "IPIP-50 (Public Domain)"}. Antworte spontan und ehrlich.
        </p>
        <div className="mt-2 text-sm">
          {long ? (
            <Link href={`/assessments/${params.personId}/bigfive`} className="text-accent-soft underline">→ Kurzversion (50 Items)</Link>
          ) : (
            <Link href={`/assessments/${params.personId}/bigfive?long=1`} className="text-accent-soft underline">→ Langversion (100 Items, genauer)</Link>
          )}
        </div>
      </header>
      ...
      <div className="sticky top-[57px] z-10 rounded-xl border border-white/10 bg-ink/90 p-3 backdrop-blur">
        <div className="mb-1 flex justify-between text-xs text-slate-400">
          <span>{answered} / {items.length} beantwortet</span>
          <span>{progress}%</span>
        </div>
      ...
      <div className="sticky bottom-4 flex items-center justify-between rounded-xl border border-white/10 bg-ink/90 p-3 backdrop-blur">
        <span className="text-sm text-slate-400">{allDone ? "Alle Fragen beantwortet ✓" : `Noch ${items.length - answered} offen`}</span>
